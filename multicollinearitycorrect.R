...
mat_a = subset(dat, select = -c(median_house_value))
...
data3 = dat[, -which(colnames(dat) %in% highlyCorCol)]
dim(data3)
...
df=cbind(dat$median_house_value,data3)
df=as.data.frame(df)
fit2=lm(median_house_value~.,data=df)
vif(fit2)
#alias(lm(type~.,data=df))
